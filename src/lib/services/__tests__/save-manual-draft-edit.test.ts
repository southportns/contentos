...
describe('P0.5.1 — saveManualDraftEdit', () => {
  ...
  describe('Test J — ACTIVE_DRAFT_CONFLICT mapping', () => {
    it('maps ACTIVE_DRAFT_CONFLICT to user-friendly error', async () => {
      mockCreateManualEditDraft.mockRejectedValue(new Error('ACTIVE_DRAFT_CONFLICT'))

      const result = await saveManualDraftEdit('draft_v4', '测试活跃草稿冲突时的错误映射逻辑是否正确工作的内容需要超过50个字。')
