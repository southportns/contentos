/*
 * P0.4.3/P0.4.4/P0.4.5/P0.4.8 — Draft Version Restore Repository Tests
 *
 * Tests verify:
 ...
 *   24. P0.4.8 — getActiveDraft resolves by ID or falls back to highest version
 *   25. P0.4.8 — setActiveDraft with ownership validation and cross-topic protection
 *   26. P0.4.8 — Restore auto-sets active draft in same transaction
 */
...
function setupTransactionMock(
  ...
  mockTransaction.mockImplementation(async (callback: (tx: { draft: { findUnique: ReturnType<typeof vi.fn>; findFirst: ReturnType<typeof vi.fn>; create: ReturnType<typeof vi.fn> }; topic: { update: ReturnType<typeof vi.fn> } }) => Promise<unknown>) => {
    ...
    const tx = {
      ...
      topic: {
        update: vi.fn().mockResolvedValue({ id: resolvedTopicId, activeDraftId: createdDraft.id }),
      },
    }
    return callback(tx)
  })
...
describe('P0.4.3/P0.4.4 — createRestoredDraft', () => {
  ...
  describe('Test D — Duplicate Version (P2002 detected)', () => {
    it('detects P2002 and throws DRAFT_VERSION_CONFLICT after max retries', async () => {
      ...
      await expect(topicRepository.createRestoredDraft('draft_source_1', 'default')).rejects.toThrow('DRAFT_VERSION_CONFLICT')
  ...
  describe('Test E — Retry Success (attempt 1 conflict, attempt 2 success)', () => {
    it('succeeds on second attempt after first P2002 conflict', async () => {
      ...
      mockTransaction.mockImplementation(async (callback: (tx: { draft: { findFirst: ReturnType<typeof vi.fn>; create: ReturnType<typeof vi.fn> }; topic: { update: ReturnType<typeof vi.fn> } }) => Promise<unknown>) => {
        callCount++
        if (callCount === 1) { throw createP2002Error() }
        const tx = {
          draft: {
            findFirst: vi.fn().mockResolvedValue({ version: 5 }),
            create: vi.fn().mockResolvedValue({
              id: 'draft_new_6', topicId: 'topic_1', version: 6,
              parentDraftId: sourceDraft.id, title: sourceDraft.title,
              content: sourceDraft.content, outline: sourceDraft.outline,
              status: 'DRAFT', wordCount: sourceDraft.content.length,
              createdAt: new Date('2026-09-20T10:00:00Z'), updatedAt: new Date('2026-09-20T10:00:00Z'),
            }),
          },
          topic: { update: vi.fn().mockResolvedValue({ id: 'topic_1', activeDraftId: 'draft_new_6' }) },
        }
        return callback(tx)
      })
      const result = await topicRepository.createRestoredDraft('draft_source_1', 'default')
      ...
      mockTransaction.mockImplementation(async (callback: (tx: { draft: { findFirst: ReturnType<typeof vi.fn>; create: ReturnType<typeof vi.fn> }; topic: { update: ReturnType<typeof vi.fn> } }) => Promise<unknown>) => {
        const tx = {
          draft: {
            ...
            create: vi.fn().mockResolvedValue({
              id: 'draft_new', topicId: 'topic_1', version: 1,
              parentDraftId: sourceDraft.id, title: sourceDraft.title,
              content: sourceDraft.content, outline: sourceDraft.outline,
              status: 'DRAFT', wordCount: sourceDraft.content.length,
              createdAt: new Date('2026-09-20T10:00:00Z'), updatedAt: new Date('2026-09-20T10:00:00Z'),
          ...
          topic: { update: vi.fn().mockResolvedValue({ id: 'topic_1', activeDraftId: 'draft_new' }) },
        }
        return callback(tx)
      })
      const result = await topicRepository.createRestoredDraft('draft_source_1', 'default')
  ...
  describe('Ownership Validation', () => {
    it('throws OWNERSHIP_DENIED when user does not own the draft', async () => {
      ...
      mockDraftFindUnique.mockResolvedValue(sourceDraft)
      await expect(topicRepository.createRestoredDraft('draft_source_1', 'default')).rejects.toThrow('OWNERSHIP_DENIED')
    })
  ...
  describe('Missing Source', () => {
    it('throws SOURCE_DRAFT_NOT_FOUND when source draft does not exist', async () => {
      ...
      await expect(topicRepository.createRestoredDraft('nonexistent_draft', 'default')).rejects.toThrow('SOURCE_DRAFT_NOT_FOUND')
    })
    ...
    it('throws TOPIC_NOT_FOUND when topic is missing', async () => {
      ...
      mockDraftFindUnique.mockResolvedValue(sourceDraft)
      await expect(topicRepository.createRestoredDraft('draft_source_1', 'default')).rejects.toThrow('TOPIC_NOT_FOUND')
    })
  })
          ...
          draft: {
            ...
            create: vi.fn().mockImplementation((args: { data: Record<string, unknown> }) => {
              ...
              return Promise.resolve({
                id: 'draft_new', topicId: 'topic_1', version: 2,
                parentDraftId: args.data.parentDraftId, title: sourceDraft.title,
                content: sourceDraft.content, outline: sourceDraft.outline,
                status: 'DRAFT', wordCount: sourceDraft.content.length,
                createdAt: new Date('2026-09-20T10:00:00Z'), updatedAt: new Date('2026-09-20T10:00:00Z'),
              })
  ...
  describe('Transaction Safety', () => {
    ...
    it('when create fails with non-P2002 error, throws immediately (no retry)', async () => {
      const sourceDraft = createMockSourceDraft({ version: 1 })
      mockDraftFindUnique.mockResolvedValue(sourceDraft)
      mockTransaction.mockImplementation(async () => { throw new Error('DB_WRITE_FAILED') })
      await expect(topicRepository.createRestoredDraft('draft_source_1', 'default')).rejects.toThrow('DB_WRITE_FAILED')
      expect(mockTransaction).toHaveBeenCalledTimes(1)
    })
  })
  ...
  describe('P0.4.5 — Test G: Parent Missing (SetNull behavior)', () => {
    it('child draft can exist with null parentDraftId (simulating SetNull)', async () => {
      const childDraft = {
        id: 'draft_child', topicId: 'topic_1', version: 2,
        parentDraftId: null, title: '子版本', content: '内容',
        outline: null, status: 'DRAFT', wordCount: 2,
        createdAt: new Date('2026-09-20T10:00:00Z'), updatedAt: new Date('2026-09-20T10:00:00Z'),
      }
        ...
        const tx = {
          draft: {
            ...
            create: vi.fn().mockImplementation((args: { data: Record<string, unknown> }) => {
              createCallData = args.data
              return Promise.resolve({
                id: 'draft_v4', topicId: 'topic_1', version: 4,
                parentDraftId: args.data.parentDraftId, title: sourceDraft.title,
                content: sourceDraft.content, outline: sourceDraft.outline,
                status: 'DRAFT', wordCount: sourceDraft.content.length,
                createdAt: new Date('2026-09-20T10:00:00Z'), updatedAt: new Date('2026-09-20T10:00:00Z'),
          ...
          topic: { update: vi.fn().mockResolvedValue({ id: 'topic_1', activeDraftId: 'draft_v4' }) },
        }
        return callback(tx)
      })
      const result = await topicRepository.createRestoredDraft('draft_v1', 'default')
        ...
        const tx = {
          draft: {
            ...
            create: vi.fn().mockImplementation((args: { data: Record<string, unknown> }) => {
              ...
              return Promise.resolve({
                id: 'draft_v4', topicId: 'topic_1', version: 4,
                parentDraftId: args.data.parentDraftId, changeType: args.data.changeType,
                changeReason: args.data.changeReason, title: sourceDraft.title,
                content: sourceDraft.content, outline: sourceDraft.outline,
                status: 'DRAFT', wordCount: sourceDraft.content.length,
                createdAt: new Date('2026-09-20T10:00:00Z'), updatedAt: new Date('2026-09-20T10:00:00Z'),
              })
  ...
  describe('P0.4.8 — Test E: getActiveDraft with explicit activeDraftId', () => {
    it('returns the draft matching activeDraftId when it exists', async () => {
      const mockTopic = {
        id: 'topic_1', activeDraftId: 'draft_v2',
        activeDraft: { id: 'draft_v2', topicId: 'topic_1', version: 2, content: 'v2 content' },
        drafts: [
          { id: 'draft_v3', topicId: 'topic_1', version: 3, content: 'v3 content' },
          { id: 'draft_v2', topicId: 'topic_1', version: 2, content: 'v2 content' },
          { id: 'draft_v1', topicId: 'topic_1', version: 1, content: 'v1 content' },
        ],
      }
      mockTopicFindUnique.mockResolvedValue(mockTopic)
      const result = await topicRepository.getActiveDraft('topic_1')
  ...
  describe('P0.4.8 — Test I: getActiveDraft fallback to highest version', () => {
    it('returns highest version draft when activeDraftId is null', async () => {
      const mockTopic = {
        id: 'topic_1', activeDraftId: null, activeDraft: null,
        drafts: [
          { id: 'draft_v3', topicId: 'topic_1', version: 3, content: 'v3 content' },
          { id: 'draft_v1', topicId: 'topic_1', version: 1, content: 'v1 content' },
        ],
    ...
    it('returns highest version when activeDraftId points to non-existent draft', async () => {
      const mockTopic = {
        id: 'topic_1', activeDraftId: 'deleted_draft', activeDraft: null,
        drafts: [
          { id: 'draft_v2', topicId: 'topic_1', version: 2, content: 'v2 content' },
          { id: 'draft_v1', topicId: 'topic_1', version: 1, content: 'v1 content' },
        ],
    ...
    it('returns null when topic has no drafts', async () => {
      const mockTopic = { id: 'topic_1', activeDraftId: null, activeDraft: null, drafts: [] }
      mockTopicFindUnique.mockResolvedValue(mockTopic)
      const result = await topicRepository.getActiveDraft('topic_1')
  ...
  describe('P0.4.8 — Test F: setActiveDraft success', () => {
    it('sets the specified draft as active', async () => {
      const mockDraft = {
        id: 'draft_v2', topicId: 'topic_1', version: 2, content: 'v2 content',
        topic: { id: 'topic_1', projectId: 'proj_1', project: { id: 'proj_1', userId: 'default' } },
      }
      mockDraftFindUnique.mockResolvedValue(mockDraft)
      mockTopicUpdate.mockResolvedValue({ id: 'topic_1', activeDraftId: 'draft_v2' })
      const result = await topicRepository.setActiveDraft('topic_1', 'draft_v2', 'default')
      expect(result.id).toBe('draft_v2')
      expect(mockTopicUpdate).toHaveBeenCalledWith({
        where: { id: 'topic_1' },
        data: { activeDraftId: 'draft_v2' },
      })
    })
  })

  describe('P0.4.8 — Test G: setActiveDraft cross-topic protection', () => {
    it('rejects draft from a different topic', async () => {
      const mockDraft = {
        id: 'draft_other', topicId: 'topic_B', version: 1, content: 'other content',
        topic: { id: 'topic_B', projectId: 'proj_1', project: { id: 'proj_1', userId: 'default' } },
      }
      mockDraftFindUnique.mockResolvedValue(mockDraft)
      await expect(topicRepository.setActiveDraft('topic_A', 'draft_other', 'default')).rejects.toThrow('ACTIVE_DRAFT_INVALID')
      expect(mockTopicUpdate).not.toHaveBeenCalled()
    })
  })

  describe('P0.4.8 — Test H: setActiveDraft ownership validation', () => {
    it('rejects draft not owned by user', async () => {
      const mockDraft = {
        id: 'draft_v1', topicId: 'topic_1', version: 1, content: 'content',
        topic: { id: 'topic_1', projectId: 'proj_1', project: { id: 'proj_1', userId: 'other_user' } },
      }
      mockDraftFindUnique.mockResolvedValue(mockDraft)
      await expect(topicRepository.setActiveDraft('topic_1', 'draft_v1', 'default')).rejects.toThrow('OWNERSHIP_DENIED')
      expect(mockTopicUpdate).not.toHaveBeenCalled()
    })
  })

  describe('P0.4.8 — Test J: Restore auto-sets active draft', () => {
    it('restore creates draft AND sets it as active in same transaction', async () => {
      const sourceDraft = createMockSourceDraft({ id: 'draft_v1', version: 1 })
      mockDraftFindUnique.mockResolvedValue(sourceDraft)
      setupTransactionMock(2, { id: 'draft_v3', version: 3 }, sourceDraft)
      const result = await topicRepository.createRestoredDraft('draft_v1', 'default')
      expect(result.version).toBe(3)
      expect(result.id).toBe('draft_v3')
      expect(mockTransaction).toHaveBeenCalledTimes(1)
    })
  })

  describe('P0.4.8 — Test K: Restore + Lineage + Evolution + Active Draft', () => {
    it('all four aspects are correctly set after restore', async () => {
      const sourceDraft = createMockSourceDraft({ id: 'draft_v2', version: 2 })
      mockDraftFindUnique.mockResolvedValue(sourceDraft)
      let createCallData: Record<string, unknown> | null = null
        ...
        const tx = {
          draft: {
            ...
            create: vi.fn().mockImplementation((args: { data: Record<string, unknown> }) => {
              createCallData = args.data
              return Promise.resolve({
                id: 'draft_v5', topicId: 'topic_1', version: 5,
                parentDraftId: args.data.parentDraftId, changeType: args.data.changeType,
                changeReason: args.data.changeReason, title: sourceDraft.title,
                content: sourceDraft.content, outline: sourceDraft.outline,
                status: 'DRAFT', wordCount: sourceDraft.content.length,
          ...
          topic: {
            update: vi.fn().mockImplementation(() => {
              ...
              return Promise.resolve({ id: 'topic_1', activeDraftId: 'draft_v5' })
            }),
          },
        }
        return callback(tx)
      ...
      expect(createCallData).not.toBeNull()
      expect(createCallData!.parentDraftId).toBe('draft_v2')
      expect(createCallData!.changeType).toBe('RESTORE')
    })
  })

  describe('P0.4.8 — Test L: Transaction atomicity (draft + active update)', () => {
    it('both draft create and topic update happen inside $transaction', async () => {
      const sourceDraft = createMockSourceDraft({ id: 'draft_v1', version: 1 })
      mockDraftFindUnique.mockResolvedValue(sourceDraft)
        ...
        const tx = {
          draft: {
            ...
            create: vi.fn().mockImplementation(() => {
              txOperations.push('draft.create')
              return Promise.resolve({
                id: 'draft_v2', topicId: 'topic_1', version: 2,
                parentDraftId: 'draft_v1', changeType: 'RESTORE',
                changeReason: '基于 v1 恢复创建', title: 't', content: 'c',
                outline: null, status: 'DRAFT', wordCount: 1,
          ...
          topic: {
            update: vi.fn().mockImplementation(() => {
              txOperations.push('topic.update')
              return Promise.resolve({ id: 'topic_1', activeDraftId: 'draft_v2' })
            }),
          },
        }
        return callback(tx)
      })
      await topicRepository.createRestoredDraft('draft_v1', 'default')
      expect(txOperations).toContain('draft.create')
      expect(txOperations).toContain('topic.update')
      expect(mockTransaction).toHaveBeenCalledTimes(1)
    })
  })

  describe('P0.4.8 — Test M: Active draft update failure rolls back draft creation', () => {
    it('if topic.update throws, entire transaction is rolled back', async () => {
      const sourceDraft = createMockSourceDraft({ id: 'draft_v1', version: 1 })
      mockDraftFindUnique.mockResolvedValue(sourceDraft)
      mockTransaction.mockImplementation(async (callback: (tx: { draft: { findFirst: ReturnType<typeof vi.fn>; create: ReturnType<typeof vi.fn> }; topic: { update: ReturnType<typeof vi.fn> } }) => Promise<unknown>) => {
        const tx = {
