...
// P0.3.8.4.1 — Approve Strategy API
// Transitions strategy from 'pending' → 'approved' (server-side enforcement)
//
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Conditional update: only transitions pending → approved
    // Returns false if state was already changed (race condition protection)
    const updated = await contentService.approveStrategy(input.strategyId)
      ...
      return NextResponse.json(
        {
          ...
          message: `Cannot approve strategy in '${strategy.approvalStatus}' state. Only 'pending' strategies can be approved.`,
        },
        { status: 409 },
      )
    }
