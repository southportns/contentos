 ...
 *
 * Verifies that the calibrated parameters (TopK=5, Threshold=0.35)
...
describe('P0.3.4 Production Retrieval Defaults', () => {
  ...
  it('should have DEFAULT_SIMILARITY_THRESHOLD = 0.35', () => {
...
describe('P0.3.4 TopK Default Behavior', () => {
  it('should return at most 5 results by default', async () => {
    ...
    // Default TopK = 5, Threshold = 0.35
    // Entries with similarity >= 0.35: 0.9, 0.8, 0.7, 0.6, 0.5, 0.4 (6 entries, TopK limits to 5)
    expect(response.results.length).toBe(5);
  });
});

describe('P0.3.4 Threshold Default Behavior', () => {
  it('should filter results with similarity < 0.35', async () => {
    const provider = new FixedVectorProvider([1, 0]);

    // Create entries with similarities: 0.60, 0.55, 0.42, 0.36, 0.35, 0.34, 0.20
    const similarities = [0.60, 0.55, 0.42, 0.36, 0.35, 0.34, 0.20];
    ...
    const response = await retriever.retrieve({
      ...
      // No min_similarity - should use default 0.35
    });

    // Expected: 0.60, 0.55, 0.42, 0.36, 0.35 (>= 0.35)
    // Filtered: 0.34, 0.20 (< 0.35)
    expect(response.results.length).toBe(5);
    ...
    expect(similarities_in_result).toContain(0.36);
    expect(similarities_in_result).toContain(0.35);
    expect(similarities_in_result).not.toContain(0.34);
    expect(similarities_in_result).not.toContain(0.20);
  });

  it('should include similarity exactly equal to 0.35', async () => {
    const provider = new FixedVectorProvider([1, 0]);

    const entries = [
      { id: 'KU_001', vector: [0.35, Math.sqrt(1 - 0.1225)] },
    ];
    ...
    // similarity = 0.35 should be included (>= 0.35)
    expect(response.results.length).toBe(1);
    expect(response.results[0].similarity).toBeCloseTo(0.35, 2);
  });

  it('should exclude similarity just below 0.35', async () => {
    const provider = new FixedVectorProvider([1, 0]);

    const entries = [
      { id: 'KU_001', vector: [0.34, Math.sqrt(1 - 0.1156)] },
    ];
    ...
    // similarity = 0.34 should NOT be included (< 0.35)
...
describe('P0.3.4 Threshold Before TopK Ordering', () => {
  it('should apply threshold BEFORE TopK', async () => {
    ...
    // After threshold (>= 0.35): 0.90, 0.85, 0.80, 0.70, 0.65
    ...
    const response = await retriever.retrieve({
      query: 'test query',
      // Use defaults: TopK=5, Threshold=0.35
    });
...
describe('P0.3.4 Irrelevant Query Returns Empty', () => {
  it('should return empty results for completely irrelevant query', async () => {
    ...
    // All entries have similarity < 0.35
...
describe('P0.3.4 Custom Parameter Override', () => {
  ...
  it('should respect custom min_similarity parameter', async () => {
    ...
    const response = await retriever.retrieve({
      query: 'test query',
      limit: 10,
      min_similarity: 0.40, // Override default 0.35
    });
...
describe('P0.3.4 Results Sorted by Similarity Descending', () => {
  it('should return results sorted by similarity descending', async () => {
    ...
    const similarities = [0.50, 0.90, 0.45, 0.70, 0.60];
    const entries = similarities.map((sim, i) => ({
      id: `KU_${String(i + 1).padStart(3, '0')}`,
      vector: [sim, Math.sqrt(1 - sim * sim)],
    }));
    ...
    // Sorted descending: 0.90, 0.70, 0.60, 0.50, 0.45
    ...
    expect(response.results[4].similarity).toBeCloseTo(0.45, 2);
  });
});
