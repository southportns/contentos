...
import { compareDraftVersions, canCompareVersions, swapCompareVersions, getDefaultCompareVersions } from './draft-version-utils'
...
/**
 * Inner component — all hooks called unconditionally at top level.
 * Parent (DraftVersionCompare) handles the "not enough drafts" case.
 */
function DraftVersionCompareContent({ drafts }: DraftVersionCompareProps) {
  const defaults = getDefaultCompareVersions(drafts)

  const [versionA, setVersionA] = useState<number>(defaults.versionA!)
  const [versionB, setVersionB] = useState<number>(defaults.versionB!)
  ...
  const handleSwap = () => {
    const swapped = swapCompareVersions(versionA, versionB)
    setVersionA(swapped.versionA)
    setVersionB(swapped.versionB)
  }

  const canDiff = canCompareVersions(versionA, versionB)
  ...
  return (
    <div className="space-y-4">
      ...
      {canDiff && <div className="flex flex-wrap gap-3 text-xs"><span className="text-green-600 dark:text-green-400">+ {addedCount} 行新增</span><span className="text-red-600 dark:text-red-400">- {removedCount} 行删除</span><span className="text-muted-foreground">{unchangedCount} 行未变化</span></div>}
      {canDiff ? (<div><div className="text-xs text-muted-foreground mb-2 flex items-center gap-1.5"><ArrowLeftRight className="size-3" />内容对比 (B → A)</div><DraftVersionDiff diffLines={diffLines} /></div>) : (<Card><CardContent className="py-6 text-center text-sm text-muted-foreground">请选择两个不同的版本进行对比</CardContent></Card>)}
    </div>
  )
}

/**
 * P0.4.2 — Draft Version Compare Panel
 *
 * Read-only comparison. Handles the "fewer than 2 drafts" case
 * WITHOUT violating React Hooks rules — all hooks live in DraftVersionCompareContent.
 */
export function DraftVersionCompare({ drafts }: DraftVersionCompareProps) {
  if (!drafts || drafts.length < 2) {
    return (<Card><CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><ArrowLeftRight className="size-4" />版本对比</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">{drafts && drafts.length === 0 ? '还没有生成内容版本' : '至少需要两个版本才能进行对比'}</p></CardContent></Card>)
  }
  return <DraftVersionCompareContent drafts={drafts} />
}
