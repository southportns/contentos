...
// ─── Manual Content Expiration Logic ─────────────────────
// Extracted as pure function for testability (P0.3.9.3 P5)

/**
 * Computes a signature string from refineData fields that should trigger
 * manual content expiration when they change.
 */
export function computeRefineSignature(
  refineData: { content?: string; title?: string; hook?: string } | null | undefined
): string {
  return `${refineData?.content ?? ''}\u0000${refineData?.title ?? ''}\u0000${refineData?.hook ?? ''}`
}

/**
 * Decides whether manual content/title should be expired based on
 * previous and next refineData signatures.
 */
export function shouldExpireManualContent(prevSignature: string, nextSignature: string): boolean {
...
export function StepRefine({
  ...
  // P0.3.9.3: Manual Content Expiration — when AI produces new refineData, stale manual edits are cleared
  const lastRefineSignatureRef = useRef(computeRefineSignature(refineData))

  useEffect(() => {
    const nextSignature = computeRefineSignature(refineData)

    if (shouldExpireManualContent(lastRefineSignatureRef.current, nextSignature)) {
      setManualContent(null)
      setManualTitle(null)
    }

    lastRefineSignatureRef.current = nextSignature
  }, [
    refineData?.content,
    refineData?.title,
    refineData?.hook,
