...
float lineFn(vec2 st, float width, float perc, vec2 mouse, float time, float amplitude, float distance, float pixelVal) {
  ...
  float line_start = smoothstep(
    y + (width / 2.0) + (u_line_blur * pixelVal * blur),
    y,
    st.y
  );
...
void main() {
  ...
  float pixelVal = uPixelRatio / max(iResolution.x, iResolution.y);
...
const Threads = ({
  ...
  useEffect(() => {
    ...
    function resize() {
      const { clientWidth, clientHeight } = container ?? { clientWidth: 0, clientHeight: 0 }
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      if (now - lastMouseEvent < 16) return
      lastMouseEvent = now
      const rect = container?.getBoundingClientRect() ?? { left: 0, top: 0, width: 1, height: 1 }
